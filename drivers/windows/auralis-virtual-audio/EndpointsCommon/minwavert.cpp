...
)
{
    ...
    else if (IsLoopbackPin(_Pin))
    {
        ...
        allocatedCount = &m_ulLoopbackAllocated;
    ...
    else if (IsSystemRenderPin(_Pin))
    {
        ...
        allocatedCount = &m_ulSystemAllocated;
    }
    else if (IsOffloadPin(_Pin))
    {
        streams = m_OffloadStreams;
        count = m_ulMaxOffloadStreams;
        allocatedCount = &m_ulOffloadAllocated;
    }
    ...
    if (streams != NULL)
    {
        const BOOLEAN registered = RegisterStreamPointer(
            streams, count, _Stream, allocatedCount);
        ASSERT(registered);
        if (!registered)
        {
            return STATUS_INSUFFICIENT_RESOURCES;
...
)
{
    ...
    if (streams != NULL)
    {
        const BOOLEAN removed = UnregisterStreamPointer(
            streams, count, _Stream, allocatedCount);
        ASSERT(removed);
        if (!removed)
        {
...
#pragma code_seg()
BOOLEAN
CMiniportWaveRT::RegisterStreamPointer
(
    _Inout_updates_(Count) PCMiniportWaveRTStream* Streams,
    _In_ ULONG Count,
    _In_ PCMiniportWaveRTStream Stream,
    _Inout_ PULONG AllocatedCount
)
{
    if (Streams == NULL || Stream == NULL || AllocatedCount == NULL)
    {
        return FALSE;
    }
    ...
    KIRQL oldIrql;
    KeAcquireSpinLock(&m_StreamListLock, &oldIrql);
    ...
    KeReleaseSpinLock(&m_StreamListLock, oldIrql);
    return registered;
}
...
#pragma code_seg()
BOOLEAN
CMiniportWaveRT::UnregisterStreamPointer
(
    _Inout_updates_(Count) PCMiniportWaveRTStream* Streams,
    _In_ ULONG Count,
    _In_ PCMiniportWaveRTStream Stream,
...
#pragma code_seg()
BOOLEAN
CMiniportWaveRT::CopySystemRenderMix
(
    _Out_writes_bytes_(ByteCount) BYTE* Destination,
    _In_ ULONG ByteCount,
    _In_ PWAVEFORMATEX LoopbackFormat,
    _Inout_ ULONGLONG* SourcePosition
)
{
    if (Destination == NULL || ByteCount == 0 || LoopbackFormat == NULL ||
        SourcePosition == NULL || m_LoopbackProtection == CONSTRICTOR_OPTION_MUTE)
    {
    ...
    KIRQL streamListIrql;
    KeAcquireSpinLock(&m_StreamListLock, &streamListIrql);

    for (ULONG index = 0; index < m_ulMaxSystemStreams; ++index)
    {
        PCMiniportWaveRTStream source = m_SystemStreams[index];
        if (source == NULL)
        {
            continue;
        }

        KeAcquireSpinLockAtDpcLevel(&source->m_PositionSpinLock);
        const BOOLEAN compatible = source->m_KsState == KSSTATE_RUN &&
            source->m_pDmaBuffer != NULL && source->m_ulDmaBufferSize > 0 &&
            source->m_pWfExt != NULL &&
            source->m_pWfExt->Format.nSamplesPerSec == LoopbackFormat->nSamplesPerSec &&
            source->m_pWfExt->Format.nChannels == LoopbackFormat->nChannels &&
            source->m_pWfExt->Format.wBitsPerSample == LoopbackFormat->wBitsPerSample &&
            source->m_pWfExt->Format.nBlockAlign == LoopbackFormat->nBlockAlign;

        if (compatible)
        {
            const ULONGLONG newestPosition = source->m_ullLinearPosition;
            const BOOLEAN outsideCurrentRing = *SourcePosition != MAXULONGLONG &&
                (newestPosition >= *SourcePosition
                    ? newestPosition - *SourcePosition > source->m_ulDmaBufferSize
                    : *SourcePosition - newestPosition > source->m_ulDmaBufferSize);
            if (*SourcePosition == MAXULONGLONG || outsideCurrentRing)
            {
                // A loopback client may attach long after rendering began. Start
                // at the current hardware-consume position. Also resync after
                // an overrun or a render-stream restart instead of replaying
                // stale ring contents at an unrelated modulo offset.
                *SourcePosition = newestPosition;
            }
            ...
            while (remaining > 0)
            {
                const ULONG sourceOffset = static_cast<ULONG>(
                    *SourcePosition % source->m_ulDmaBufferSize);
                const ULONG run = min(remaining, source->m_ulDmaBufferSize - sourceOffset);
                RtlCopyMemory(
                    Destination + destinationOffset,
                    source->m_pDmaBuffer + sourceOffset,
                    run);
                *SourcePosition += run;
                destinationOffset += run;
                remaining -= run;
            }
            copied = TRUE;
        }

        KeReleaseSpinLockFromDpcLevel(&source->m_PositionSpinLock);
        if (copied)
        {
            break;
        }
    ...
    KeReleaseSpinLock(&m_StreamListLock, streamListIrql);
    if (!copied)
    {
        RtlZeroMemory(Destination, ByteCount);
    }
