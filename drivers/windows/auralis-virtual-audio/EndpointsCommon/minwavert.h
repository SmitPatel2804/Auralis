...
public:
    ...
    BOOLEAN UnregisterStreamPointer
    (
        _Inout_updates_(Count) PCMiniportWaveRTStream* Streams,
        _In_ ULONG Count,
        _In_ PCMiniportWaveRTStream Stream,
    ...
    // Copy the audio-engine render mix into a loopback stream. The upstream
    // SysVAD sample generates a test tone here; Auralis needs the PCM that
    // applications actually rendered to the virtual endpoint.
    BOOLEAN CopySystemRenderMix
    (
