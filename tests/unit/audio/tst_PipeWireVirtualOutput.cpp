...
private slots:
    ...
    void delayBridgeUsesLoopbackTargetDelay()
    {
        ...
        QVERIFY(args.contains("node.virtual = false"));
        QVERIFY(args.contains("node.autoconnect = false"));
        QVERIFY(args.contains("node.autoconnect = true"));
        QVERIFY(args.contains("target.object = \"bluez_output.one\""));
        QVERIFY(!args.contains("node.passive = true"));
        QVERIFY(!args.contains("auralis.delay.endpoint"));
        QCOMPARE(auralis::audio::auralisDelayBridgeCaptureNodeName(QStringLiteral("dest-a")),
                 QStringLiteral("auralis_delay_dest_a"));
        QCOMPARE(auralis::audio::auralisDelayBridgePlaybackNodeName(QStringLiteral("dest-a")),
                 QStringLiteral("auralis_delay_dest_a.source"));
    }
};
