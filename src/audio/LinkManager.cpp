...
QVector<OwnedLink> LinkManager::createLinks(const QString& routeId, const ResolvedRoutePlan& plan, RouteErrorInfo* error)
{
    ...
    for (const ResolvedPortPair& pair : plan.pairs) {
        ...
        qCInfo(auralisAudio) << "LinkManager owned link route=" << routeId << "pw=" << owned.globalId
...
{
    if (backend_ != nullptr && link.globalId != 0) {
        backend_->destroyOwnedLink(link.globalId);
    }
    link.globalId = 0;
}
